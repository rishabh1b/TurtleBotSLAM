...
void LineMatcher::BruteForcePairs(std::vector<Line> l_old, std::vector<Line> l_new, std::vector<LineMatcher::Pair> &matched)
{
  int sz1 = l_old.size();
  int sz2 = l_new.size();
 ...
 double curr_min_threshold;
  for (i = 0; i < sz1; i++)
  {
     int curr_min_thresh = 100;
     for (j = 0; j < sz2; j++)
     {
        double err_ = l_new[j].getError(l_old[i]);
        if (err_ < 10 && err_ < curr_min_thresh) // TODO: Put a Parameter in the Parameter Server for thresh_matching_lines
           {
